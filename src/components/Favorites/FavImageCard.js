...
const FavImageCard = ({removeFavorite, image}) => {
    return (
        <div className="my-1 px-1 w-full md:w-1/2 lg:my-4 lg:px-4 lg:w-1/3 slide-in-blurred-top ">
            <article className="overflow-hidden rounded-md shadow-lg dark:bg-gray-700">
            ...
            <footer className="flex items-center justify-between leading-none p-2 md:p-4">
                ...
                <a className="flex items-center no-underline hover:underline text-black dark:text-white" href={"user/"+image.user.username} >
                    <img className="block rounded-full" src={image.user.profile_image.small} />
                    <p className="ml-2 text-sm">
                    {image.user.username}
                    </p>
                </a>
                <FontAwesomeIcon onClick={()=>removeFavorite(image)} className="dark:text-white cursor-pointer" icon={faMinusCircle} />
            </footer>
            </article>
        </div>
    )
